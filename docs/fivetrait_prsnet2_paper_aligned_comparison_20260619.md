...
- 原文效果对比 / Article comparison: `docs/prsnet2_article_vs_our_dataset_comparison_20260619.md`
...
PRSNet-2（SNP→基因→表型层级图神经网络）achieved the highest mean macro-F1（各性状宏平均F1均值）among the paper-aligned baselines, outperforming both a train-fold GWAS top-k additive PRS baseline（只用训练集GWAS前k个SNP的加性多基因评分基线）and an SNP-MLP neural baseline（SNP输入的普通多层感知机神经网络基线） under the leakage-aware（防数据泄漏）five-trait ordinal benchmark（五性状有序分类基准）.
