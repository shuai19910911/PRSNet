...
2. SNP-MLP class-balanced loss 已证明可把 macro-F1 提到 `0.3606`，但牺牲 accuracy/MAE；下一步做 weighted-loss 强度退火或 ordinal/class-balanced 混合目标，寻找 macro-F1 与 accuracy 的折中。
3. 已完成 `max_snps_per_gene=16/32/64/128` 的 seed42 对齐消融；指标近似持平，暂不作为主瓶颈继续深挖。
4. body-only、±2kb、±10kb、nearest-gene mapping 输入已生成；下一步重建对应 gene graph / model-input manifest，并优先选择 1–2 个差异最大的 mapping 做 bounded pilot。
