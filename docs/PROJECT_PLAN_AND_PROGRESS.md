...
1. STRING rice graph 已构建并通过 CPU 读取 smoke；下一步在可用 `gpu10` shell 上运行 `string_v12_min700_top20` 的 top512 + trait_attention + GraphEncoder 15 epoch bounded pilot，并与 chr-neighbor / prefix random 三 seed结果对齐比较。
2. 已完成 `max_snps_per_gene=16/32/64/128` 的 seed42 对齐消融；指标近似持平，暂不作为主瓶颈继续深挖。
3. 已完成 SNP-MLP baseline 初版；下一步可补 early stopping / dropout / weight decay 网格以判断过拟合是否可控。
4. 继续构建 Plant Reactome/KEGG pathway、co-expression atlas 等外部 biological graph，用于替换或融合当前 baseline / STRING graph。
