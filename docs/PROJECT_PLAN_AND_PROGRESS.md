...
1. 已完成 chr-neighbor vs prefix2048_random 三个 seed（42/43/44）对照；结果不支持宣称当前轻量 GraphEncoder 的染色体邻接结构特异增益。下一步优先引入更强外部 biological graph（STRING rice、Plant Reactome/KEGG/pathway、co-expression atlas）再评估图结构贡献。
