...
- [2026-06-14 08:58:48 CST] Cron 补齐 `chr_string_v12_min700_top20_fusion` seed43/44 对齐复跑：继续在 `gpu10` 物理 GPU0 上运行同一配置（top512 + trait_attention + GraphEncoder、genes=2048、hidden_dim=64、batch=16、最多 15 epoch、50 steps/epoch、early_stop_patience=5）。seed43：`status=ok`、`graph_edges_used=20850`、early stop 后记录 best epoch `10`、best val loss `0.3796`、final val loss `0.3832`、accuracy `0.5865`、MAE `0.5893`、macro-F1 `0.2755`、Spearman `0.2545`；seed44：`status=ok`、`graph_edges_used=20850`、early stop 于 14 epoch、best epoch `9`、best val loss `0.3757`、final val loss `0.3943`、accuracy `0.5788`、MAE `0.5983`、macro-F1 `0.2908`、Spearman `0.2087`。两组 manifest 均通过 `json.tool`，`checkpoint_best.pt`/`checkpoint_last.pt` 均非空（约 1.52 MB，本地数据区，不上传 GitHub）。综合 seed42/43/44：fusion 提升了 STRING 在 bounded subset 的边覆盖，但指标仍与 chr-neighbor / prefix random 非常接近；当前不能声称 STRING+chr 融合有稳定结构特异增益。
- [2026-06-14 09:21:36 CST] Cron 复核 Phase 5 model-input smoke 并推进 SNP-MLP 正则/早停对照：`squeue -j 8562921` 已无活动作业，`sacct` 返回 `8562921|rgf_input_smoke|cu|COMPLETED|0:0|00:00:08`；manifest/report 再次验证 `status=ok`、`X=3000x365710`、`Y/mask=3000x35`、10 core traits、graph `34139` nodes / `341030` directed edges、random split train/val/test=`1586/340/340`。`PRSNet` 环境 PyTorch 为 `2.6.0+cu124`，登录节点无 GPU，`gpu10` 物理 GPU0 为 A100-40GB 且空闲；本 tick 选择先跑 CPU 轻量 baseline 正则对照。SLURM 作业 `8562946` 在 `cu` 分区完成（`COMPLETED`, exit `0:0`, elapsed `00:01:04`, batch MaxRSS `1583448K`）：SNP-MLP top512/trait union `4218` SNP，hidden_dim `256`、dropout `0.2`、weight_decay `1e-4`、early_stop_patience `3`，4 epoch 早停，best epoch `1`，best val loss `0.8913`、accuracy `0.6244`、MAE `0.5673`、macro-F1 `0.3167`、Spearman `0.2765`。与初版 SNP-MLP 的较高 macro-F1 但过拟合现象相比，本组正则化改善 accuracy/MAE 接近 tree baseline，但 macro-F1 下降；后续如继续 baseline 网格，应明确以 val loss/accuracy/MAE/macro-F1 多指标权衡，不只追单一 macro-F1。

## 8. 下一步执行优先级

1. 不再把当前 chr / prefix random / STRING / fusion 的轻量 GraphEncoder 结果解释为图结构特异增益；后续若继续图方向，应转向 Plant Reactome/KEGG pathway、co-expression atlas，或重新设计更强的 graph encoder / relation-aware message passing。
2. 若继续 STRING，应先放宽 STRING 阈值或使用更适合 bounded subset 的拓扑选择，再与 prefix random 做多 seed 对照。
3. 已完成 `max_snps_per_gene=16/32/64/128` 的 seed42 对齐消融；指标近似持平，暂不作为主瓶颈继续深挖。
4. 已完成 SNP-MLP baseline 初版与一组 early stopping/dropout/weight decay 正则对照；正则组 accuracy/MAE 改善但 macro-F1 下降，后续若继续 baseline 网格需多指标权衡。
